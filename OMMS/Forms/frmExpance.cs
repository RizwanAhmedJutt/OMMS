...
namespace OMMS
{
    public partial class frmExpance : Form
    {
        ...
        public void auto()
        {
            string sql = "select max(VocNo) from Vouchers where VocType= '"+VoucherType.EV+"' ";
            reader = db.selectQuery(sql);
            if (reader.Read())
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (db.Execute(sql) > 0)
            {
                 String query = "insert into Vouchers(VocNo,VocType,VocDate) values(" + txtVocNo.Text + ",'" + VoucherType.EV+ "','" + dpExpence.Value.ToString("yyyy-MM-dd") + "')";
                 if (db.Execute(query) > 0)
                 {
                     //Debit Entry for sales entery
                     String Narration = "Cash Paid For " + comboExpenceType.Text;
                     string sql3 = "insert into VoucherDetails(VocNo,VocType,AcCode,Debit,Credit,Narration) values(" + txtVocNo.Text + ",'" + VoucherType.EV+ "',"+Accounts.ExpenceAccount.GetHashCode()+"," + txtExpenceAmount.Text + ",0,'" + Narration + "')";
                     if (db.Execute(sql3) > 0)
                     {
                         sql3 = "insert into VoucherDetails(VocNo,VocType,AcCode,Debit,Credit,Narration) values(" + txtVocNo.Text + ",'" + VoucherType.EV+ "',"+txtCashAcCode.Text+",0," + txtExpenceAmount.Text + ",'" + Narration + "')";
                         db.Execute(sql3);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (db.Execute(sql) > 0)
            {
                    //Debit Entry for sales entery
                    String Narration = "Cash Paid For " + comboExpenceType.Text;
                    string sql3 = "update  VoucherDetails set Debit=" + txtExpenceAmount.Text + ",Credit=0,Narration ='" + Narration + "' where VocNo=" + txtVocNo.Text + " AND VocType='" + VoucherType.EV + "' AND AcCode=" + Accounts.ExpenceAccount.GetHashCode() + "";
                    if (db.Execute(sql3) > 0)
                    {
                        string sql4 = "update  VoucherDetails set Debit=0,Credit=" + txtExpenceAmount.Text + ",Narration ='" + Narration + "' where VocNo=" + txtVocNo.Text + " AND VocType='" + VoucherType.EV+ "' AND AcCode=1";
                        db.Execute(sql4);
            ...
            try
            {
                ...
                if (e.ColumnIndex == 6)
                {
                    ...
                    if (dig == DialogResult.Yes)
                    {
                        ...
                        string sql1 = "delete from Vouchers where VocNo=" + dataGridView1.Rows[e.RowIndex].Cells[0].Value + " and VocType='" + VoucherType.EV+ "'";
                        string sql2 = "delete from VoucherDetails where  VocNo=" + dataGridView1.Rows[e.RowIndex].Cells[0].Value + " and VocType='" + VoucherType.EV+ "'";
                        if (db.Execute(sql) > 0)
                        {
